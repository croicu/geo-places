...
8. Follow up with the geo-builder team on the `--noninvasive` flag requested in `docs/CLI.md` (designer mode currently pulls real acquired data into `--in`, which `scripts/clean_public.py` works around locally). Also flagged there: `pull.py` should always write a relative `catalogUrl` locally (not persist an absolute one from the service), and `assetsUrl` in `settings.json` is currently inert — the pull origin is actually derived from `designUrl`.
