...
9. **Deploy time/reliability will degrade as more cities are added — see [issue #6](https://github.com/croicu/geo-places/issues/6).** `cd.yaml` runs one atomic `geo-builder` call against the full catalog on every tag push, always fresh against the public Overpass API; the `v0.1.5` release (5 areas, 25 acquisition layers + void computation) took ~13 minutes and the build already retried `429`/`504` responses from Overpass. More areas means more wall-clock time and a higher chance some single layer permanently exhausts retries and fails the whole release (atomic — no partial `--out`). Cloudflare Pages storage is **not** the constraint (~1,600+ more areas of headroom on file count alone, no total size cap) — this is purely a build-time/Overpass-reliability problem. Candidate direction (not decided): split `cd.yaml` into a per-area matrix job, run in parallel, merge into `out/` before deploying — narrows the blast radius of a flaky retry and flattens wall-clock time, but doesn't remove the "every area must succeed" requirement, and needs per-area catalog subsets assembled in CI since `geo-builder` itself only takes one whole-catalog call. Discuss and pick a direction before implementing.
