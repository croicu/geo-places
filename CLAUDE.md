...
settings.local.json          ← hand-authored, gitignored: local overrides (e.g. debug: true — diagnostics only, see Hard Architecture Rules)
...
tests/data/                     ← synthetic area (catalog.json, areas/citest/) + tests/data/providers/fake.json (canned Overpass response) — mirrors geo-builder's own tests/data/providers/fake.json convention. ci.yaml builds this, not public/, to avoid live Overpass on every push
...
public/                        ← hand-authored input only, committed
  catalog.json                  ← every area, ship and otherwise — one catalog, period (see Hard Architecture Rules on "group")
  areas/
    prague/
...
out/                            ← generated deploy artifact, gitignored, never committed
  catalog.json                    ← copied through from public/
...
- `public/catalog.json` and `public/areas/*/manifest.json` are the only hand-edited inputs. Never hand-edit anything under `out/` — always regenerated by `geo-builder`, and gitignored anyway.
...
- **There is one catalog, and everything in it ships.** `catalog.debug.json` used to exist as a separate file geo-builder would resolve to instead of `catalog.json` when `settings.local.json`'s `debug: true` — that mechanism is gone (see `tasks/area_grouping.md`; geo-builder dropped `debug` as a parameter to `load_catalog`/`save_catalog`/`_resolve_catalog_url` entirely, commit `f9ff202`). `debug` in `settings.json`/`settings.local.json` now only affects diagnostics (full exception tracebacks, `./build/` snapshots, the WebView2 remote-debugging port, and appending `?debug=1` to `designUrl`) — it has **no effect on which catalog gets built or what area selection happens**. Any area that only makes sense during development/testing (e.g. `redmond`) is a normal entry in `public/catalog.json`, tagged with an optional `"group": ["debug"]` array — `geo-browser` is what decides whether to show it (client-side query-string filtering on `group`), not the build. This means **every** area in `public/catalog.json`, `group`-tagged or not, is acquired and deployed by `cd.yaml` like anything else — there's no more "debug area never actually gets built in production" gap. Fast local iteration on one area without re-acquiring everything else is now the same tool used for CD's incremental publish: `--rebuild <id>` against a `--in` that already has the rest cached (see CI/CD section), not a separate small catalog.
...
```json
{
  ...
  "areas": [
    ...
    },
    {
      "id": "redmond",
      "name": "Redmond",
      "bbox": [-122.13, 47.66, -122.10, 47.68],
      "minRadiusPx": 32,
      "maxRadiusPx": 512,
      "liveMapRadiusPx": 640,
      "manifestUrl": "./areas/redmond/manifest.json",
      "group": ["debug"]
    }
  ]
}
...
`group` (optional, array of strings) tags an area for client-side filtering in `geo-browser` — it has no effect on the build (every area in `catalog.json` is always acquired and deployed regardless of `group`). `"debug"` isn't a special type, just a conventional group name for areas that only make sense during development/testing (see Hard Architecture Rules above and `tasks/area_grouping.md`).
...
> 2. It fetches `{baseline_url}/build-state.json` (default: `https://raw.githubusercontent.com/croicu/geo-places-baseline/main`) — the fingerprints published by the *previous* deploy, mirrored into `croicu/geo-places-baseline` (see the Baseline artifact mirror entry below). Missing (first run) → every area is treated as changed. A changed global fingerprint → every area is treated as changed (a shared style/config change must propagate everywhere, not silently apply nowhere).
> 3. It assembles a scratch `--in`: areas whose fingerprint changed (or the explicit `areas` input, which overrides the diff entirely) are seeded from the raw `public/areas/<id>/manifest.json`; every other area is seeded by fetching its already-built manifest + `layers/*.geojson` from `geo-places-baseline`. Cloudflare Pages is never read during this phase — it's a pure deploy sink, written to only by the wrangler step at the end.
...
> **Baseline artifact mirror** (see `tasks/baseline-artifact-spec.md` for the full design writeup). After the Cloudflare Pages deploy step succeeds, `cd.yaml` runs one more step that clones `croicu/geo-places-baseline` (a separate repo, `main` branch only) using the `BASELINE_REPO_PAT` fine-grained PAT secret (scoped to that repo only, `Contents: Read and write`), `rsync -a --delete`s the just-built `out/` directory over the clone, and commits + pushes only if something changed. This gives a git-diffable snapshot of exactly what's live — and, as of the change below, is the actual read source the incremental publish step (above) diffs and pulls unchanged areas from, not just a passive record. Deployment itself always stays in `geo-places`; `geo-places-baseline` is a downstream write on the way out and a read source on the way in, but is never itself deployed to Cloudflare, and Cloudflare itself is never read from — a one-way write sink. Resolved and verified live in `v0.1.9`.
...
6. ~~Use the mock/fake Overpass provider (`geo_builder.providers.fake`) to cut network cost.~~ Resolved for CI — `ci.yaml` now builds `tests/data/` (provider: fake) instead of live Overpass, see CI/CD above. `redmond` (the `group: ["debug"]`-tagged area, see item 10) still uses `"overpass"` and hits real Overpass like every other area now that there's no separate debug catalog to give it special treatment; switching it to `provider: fake` is a possible follow-up but changes what iterating on it actually exercises.
...
10. ~~Decide on a "list of catalogs" design for future trip catalogs, and separately, the `catalog.debug.json`/ship area-id-collision hazard flagged while discussing it.~~ Superseded: geo-builder went the opposite direction (`tasks/area_grouping.md`) — one catalog, period, with an optional per-area `group` array instead of a second catalog file. This resolves the collision hazard by construction (nothing to collide with) and the debug-area-never-built-in-production gap in the same stroke. Migrated here: `redmond` moved into `public/catalog.json` as `group: ["debug"]`, `catalog.debug.json` deleted (real and CI fixture), `build.sh`/`build.cmd`/`scripts/prepare_incremental_build.py`/`tests/ci/test_catalog.py` updated accordingly. Not yet exercised against a real deploy.
