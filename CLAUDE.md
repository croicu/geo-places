...
> **Baseline artifact mirror** (see `tasks/baseline-artifact-spec.md` for the full design writeup). After the Cloudflare Pages deploy step succeeds, `cd.yaml` runs one more step that clones `croicu/geo-places-baseline` (a separate repo, `main` branch only) using the `BASELINE_REPO_PAT` fine-grained PAT secret (scoped to that repo only, `Contents: Read and write`), `rsync -a --delete`s the just-built `out/` directory over the clone, and commits + pushes only if something changed. This gives a git-diffable snapshot of exactly what's live, for future incremental-build tooling to consume — distinct from (and currently redundant with) the live-site-fetch mechanism the incremental publish step above already uses. Deployment itself always stays in `geo-places`; `geo-places-baseline` is a passive downstream write, never itself deployed to Cloudflare. Resolved and verified live in `v0.1.9`.
