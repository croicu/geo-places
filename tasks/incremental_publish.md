...
> **Superseded 2026-07-18 (see `tasks/baseline-artifact-spec.md`):** the "carried-through" fetch source for unchanged areas (and the `build-state.json` diff baseline) is no longer the live production site. It's now `croicu/geo-places-baseline`, a separate repo that `cd.yaml` mirrors `out/` into after every successful deploy. `--production-url`/`GEO_PLACES_PRODUCTION_URL` were renamed to `--baseline-url`/`GEO_PLACES_BASELINE_URL` (default `https://raw.githubusercontent.com/croicu/geo-places-baseline/main`) in `scripts/prepare_incremental_build.py`, `build.sh`, and `build.cmd`. Motivation: this removes Cloudflare Pages as a read dependency during the build phase entirely — it becomes a pure write-only deploy sink — and closes off a whole class of failure already seen once (`6b2a24b`, a live-production-fetch 403 from Cloudflare's default-User-Agent blocking, fixed for that mechanism but no longer even a relevant failure mode now that fetches go to `raw.githubusercontent.com` instead). Every other mechanic described below (fingerprinting, diffing, `--rebuild`, the scratch `--in` assembly) is unchanged — only the fetch target moved.

> **Superseded 2026-07-15 (see `tasks/area_grouping.md`):** `catalog.debug.json` is gone entirely — geo-builder dropped the debug-catalog-file concept in favor of a per-area `group` field on the one and only `public/catalog.json`. Every mention below of "ship catalog only" / copying `catalog.debug.json` through / the catalog-keyed `build-state.json` wrapper reflects the design *as originally built*, not current behavior. What actually changed: `assemble_scratch` no longer copies a second catalog file, and `build-state.json`'s schema flattened from `{"catalog.json": {...}}` to just `{...}` directly (there's nothing else to key by anymore). The fingerprinting/diffing/`--rebuild` mechanics described below are otherwise unaffected — every area in the one catalog, `group`-tagged or not, goes through the exact same incremental logic now.
